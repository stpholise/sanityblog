    ...
    < >
        <header className='flex  flex-row  item-center sticky top-0 md:static justify-between bg-[#232536] md:bg-[#232536] text-white    px-8 py-4 lg:py-3 z-50 lg:px-10'>
            <div className="logo flex items-center">
               ...
               <nav>
                <ul className={`headerNav md:flex items-center lg:flex xl:flex 2xl:flex md:flex-row lg:flex-row xl:flex-row 2xl:flex-row gap-5 flex-col  ${showMenu ? 'fixed top-14 left-0 right-0 flex-col justify-center items-center  z-40 bg-[#232536] p-3 text-left py-7 animate__animated animate__fadeInRight animate__fast' : `' absolute top-14 bg-[#232536] p-3 text-left py-7 animate__animated animate__fadeOutLeft  animate__fast left-0 right-0  `}`}>
            ...
            <button className="hidden lg:hidden xl:hidden 2xl:hidden" onClick={() => setShowMenu(!showMenu)} >
                {showMenu ?
                <img src={Xicon} />
                :<img src={Hamborger} />
            }
