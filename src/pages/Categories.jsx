...
const Categories = () => { 
    ...
    const { category, loading: loadingCategories, error:catError } = useFetchSingleCategory(id)
    ...
    <div>
        <header className="category-header w-full px-4 lg:px-0 bg-[#eee6f6] xl:h-[400px] h-fit flex  items-center justify-center">
            <div className="container mx-auto max-w-[704px] p-2 w-full flex flex-col items-left md:items-center justify-center gap-3 text-center">
               {loadingCategories ? <Skeleton /> : <h1 className="lg:text-5xl font-bold sm:text-3xl text-2xl capitalize"> {category.title}</h1>}
              {loadingCategories ? <Skeleton /> :  <p className="text-[#6D6E76] text-left md:text-center">{category.description}</p>}

                <div className="category-flex flex  justify-center items-start gap-5 w-full px-16 flex-wrap">
                    <span>BLOG</span> {loadingCategories ? <Skeleton /> : <span className='uppercase'>{category.title}</span>}
                </div>
            </div>
        </header>
